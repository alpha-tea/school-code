...
void chapter_1()
{
    printf("Chapter 1. Basic input/output, operator assign and calculate of formulas;\n\n");
    double a = 5, b = 5, c = 5, d = 0, x = 5;
    double r = (-b + 1.0 / a) / (2.0 / c);
    printf("1.15g: (-b + 1 / a) / (2 / c) = (-%.0f + 1 / %.0f) / (2 / %.0f) = %.2f\n", b, a, c, r);
    r = 1.0 / (1.0 + (a + b) / 2.0);
    printf("1.15z: 1 / (1 + (a + b) / 2) = 1 / (1 + (%.0f + %.0f) / 2) = %.2f\n", a, b, r);
    int m = 2, n = 3;
    ...
    double Pi = 3.14;
    int R = 5;
    printf("1.17g: 2 * Pi * R = 2 * %.2f * %d = %.1f\n", Pi, R, 2 * Pi * R);
    a = 20; b = 15; c = 56; d = 17;
    printf("1.17H: ((a * d) + (b * c)) / (a * d) = ((%.1f * %.1f) + (%.1f * %.1f)) / (%.1f * %.1f) = %.3f\n",
           a, d, b, c, a, d, ((a * d) + (b * c)) / (a * d));
    double m1 = 5, v = 10, g = 10, h = 50;
    printf("1.17G: ((m * v ^ 2) / 2) + (m * g * h) = ");
    r = (m1 * v * v) / 2.0 + (m1 * g * h);
    printf("((%.0f * %.0f ^ 2) / 2) + (%.0f * %.0f * %.0f) = %.0f\n", m1, v, m1, g, h, r);
    int s1 = 14, k1 = -3, d1 = 0;
    d1 = s1 + 1;
    printf("d := s + 1 = %d + 1 = %d\n", s1, d1);
    s1 = d1;
    ...
    printf("1.27: Gorizont = sqrt((R + h1) ^ 2 - R ^ 2) = sqrt((%.2f + %.2f) ^ 2 - %.2f ^ 2) = ", R1, h1, R1);
    r3 = pow((R1 + h1), 2);
    r4 = pow(R1, 2);
    printf("sqrt(%.2f - %.2f) = %.2f\n", r3, r4, sqrt(r3 - r4));
    ...
    double z = 0, x2 = 5, y = 8;
    printf("1.30a: z = x ^ 3 - 2.5 * x * y + 1.78 * x ^ 2 - 2.5 * y + 1 = ");
    ...
    sred_arif = (n1 + f) / 2.0;
    printf("1.31: sred_arif = (n + f) / 2 = (%d + %d) / 2 = %.1f;\n", n1, f, sred_arif);
    sred_geom = pow(n1 * f, 1.0 / 2.0);
    printf("\tsred_geom = pow(n * f, 1.0 / 2.0) = pow(%d * %d, 1.0 / 2.0) = %.2f\n", n1, f, sred_geom);
    int kat_1 = 3, kat_2 = 4, gip = 0, P = 0;
    gip = sqrt(pow(kat_1, 2) + pow(kat_2, 2));
    printf("1.37: Gipotenuza = sqrt(pow(kat_1, 2) + pow(kat_2, 2)) = ");
    printf("sqrt(pow(%d, 2) + pow(%d, 2)) = %d;\n", kat_1, kat_2, gip);
    P = kat_1 + kat_2 + gip;
    printf("P = kat_1 + kat_2 + gip = %d + %d + %d = %d\n", kat_1, kat_2, gip, P);
    double b1 = 5, a1 = 5;
    x = ((2.0 / (pow(a1, 2) + 25)) + b1) / (sqrt(b1) + (a1 + b1) / 2.0);
    printf("1.40: x = ((2 / (pow(a, 2) + 25)) + b) / (sqrt(b) + (a + b) / 2) = \n");
    printf("((2 / (pow(%.2f, 2) + 25)) + %.2f) / (sqrt(%.2f) + (%.2f + %.2f) / 2) = %.3f\n", a1, b1, b1, a1, b1, x);
    y = (fabs(a1) + 2 * sin(b1)) / (5.5 * a1);
    printf("y = (abs(a) + 2 * sin(b)) / (5.5 * a) = ");
    printf("(abs(%.2f) + 2 * sin(%.2f)) / (5.5 * %.2f) = %.3f\n", a1, b1, a1, y);
    double apple = 30.5, candy = 115, cookies = 20.99, c_m1 = 1, c_m2 = 2, c_m3 = 3, SUMM = 0;
    printf("1.52: SUMM = apple * m1 + candy * m2 + cookies * m3 = ");
    SUMM = apple * c_m1 + candy * c_m2 + cookies * c_m3;
    printf("%.2f * %.2f + %.2f * %.2f + %.2f * %.2f = %.2f\n", apple, c_m1, candy, c_m2, cookies, c_m3, SUMM);
    ...
    kv = cl + 273.15;
    printf("Kelvin = cl + 273.15 = %.1f + 273.15 = %.1f\n", cl, kv);
    fg = 451;
    cl = (fg - 32) / 1.8;
    printf("1.58: cl = (fg - 32) / 1.8 = (%.1f - 32) / 1.8 = %.1f\n", fg, cl);
    int t = 5, u = 2;
    ...
    t = u - t;
    printf("t = %d; u = %d\n", t, u);
    int b2 = 5, c1 = 10, a2 = 1, temp;
    printf("1.60a: b = %d; a = %d; c = %d > ", b2, a2, c1);
    temp = b2;
    b2 = c1;
    c1 = a2;
    a2 = temp;
    printf("b = %d; c = %d; a = %d;\n", b2, c1, a2);
    b2 = 5; c1 = 10; a2 = 1;
    printf("1.60b: b = %d; a = %d; c = %d > ", b2, a2, c1);
    temp = b2;
    b2 = a2;
    a2 = c1;
    c1 = temp;
