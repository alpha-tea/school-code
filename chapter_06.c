 ...
 *  !Оформление, функции и фибоначи (fibonacci) обязательновынести в библиотеку и если демонстрационный то _demo
 */
...
int arithmetic_sequence(int n, int f, int s)
{
    printf("Sequence: ");
    while (f < n) {
        printf("%d ", f);
...
int geometric_sequence(int m, int g, int z)
{
    printf("Sequence: ");
    while (g < m) {
        g *= z;
...
int number_properties(int n)
{
    int k = 0, x = 0, sum = 0, product = 1, arithmetic_mean = 0;
    ...
    arithmetic_mean = sum / k;
    sum_first_and_last = first_number + x;
    printf("a) the number of digits in it: %d\n", k);
    printf("b) the sum of its digits: %d\n", sum);
    printf("c) product of its digits: %d\n", product);
    printf("d) arithmetic mean of its digits: %d\n", arithmetic_mean);
    printf("e) the sum of the squares of its digits: %d\n", sum_square);
    printf("f) the sum of the cubes of its digits: %d\n", sum_cube);
    printf("g) its first digit: %d\n", first_number);
    printf("h) the sum of its first and last digits: %d\n\n", sum_first_and_last);
    return 0;
}

void chapter_6()
{
    printf("Chapter 6. Using conditions, computations of sequences in cycles;\n\n");
    srand(0);
    printf("6.1\n");
    ...
    n = 99;
    printf("\n\n6.8. Sequence of squares < n(%d)\n", n);
    for (int i = 1; i * i < n; ++i)
        printf("%d; ", i * i);
    printf("\n\n6.11. Sequence in the interval from 1 to 1.5\n");
    double a = 1.3;
    n = 7;
    if (a > 1.0 && a <= 1.5) {
        for (int i = 2; i < n; ++i) {
    ...
    a = 2.5;
    double summ = 30;
    printf("6.17. Sum 1 + 1 / 2 + 1 / 3 ... + 1 / n > a, a = %.2f\n", a);
    ...
    int x3 = 4; int x4 = 0;
    int x5 = 5; sum = 0;
    int x6 = 3; int x7 = 2; int x8 = 0; int x9 = 0;
    while (n > 0) {
        value = abs(n) % 10;
        n /= 10;
        if (x1 == value)
    ...
    while (n > 0) {
        ...
        if (value > x1) {
            x3 = x1;
            x1 = value;
        } else if (value > x3 && x1 > x3 && value != x1)
            x3 = value;
        if (value < x2) {
            x4 = x2;
            x2 = value;
        } else if (value < x4 && x2 < x4 && value != x2)
            x4 = value;
    }
    ...
    x3 = 33;
    if (x1 > 0 && x2 > 0 && x3 > 0) {
        int gcd_first_step = 0, gcd_second_step = 0;
        gcd_first_step = gcd(x1, x2);
        printf("GCD(a,b) = GCD(%d,%d) = %d\n", x1, x2, gcd_first_step);
        gcd_second_step = gcd(gcd_first_step, x3);
        printf("GCD(a,b,c) = GCD(%d,%d,%d) = %d\n\n", x1, x2, x3, gcd_second_step);
    } else {
    ...
    if (value < b)
        ...
        printf("value > b(%d);\n", b);
    if (x == x1)
        printf("c) Number starts with x(%d), ", x);
    else
        printf("c) Number does not start with x(%d), ", x);
    if (y1 == y2)
        printf("number ends on y(%d);\n", y1);
    else
        printf("number does not end on y(%d);\n", y1);
    if (prod_numbers < a1)
        printf("d) Product of digits less than a(%d), ", a1);
    else
        printf("d) Product of digits greater than a(%d), ", a1);
    if (value % b == 0)
        printf("number is divisible by b(%d);\n", b);
    else
        printf("number is not divisible by b(%d);\n", b);
    if (sum > m)
        printf("e) Sum numbers(%d) > m(%d), ", sum, m);
    else
        printf("e) Sum numbers(%d) < m(%d), ", sum, m);
    if (value % n == 0)
    ...
    if (k > 1) {
        if (k - 1 == x2)
            printf("digits in non-decreasing order\n\n");
        else
            printf("digits in decreasing order\n\n");
    } else
        printf("not enough digits for a sequence\n\n");
    ...
    x = 0;
    double d1 = 0;
    d1 = fabs(mass2[0] - y_val);
    for (int i = 1; i < ARRAY_SIZE; ++i) {
        double a_val = fabs(mass2[i] - y_val);
        if (a_val < d1) {
            d1 = a_val;
            x = i;
        }
    ...
    for (int i = 0, m = 0, n_val = 0, j = 0; i < total_dots * total_dots; ++i) {
        if (m <= n_val) {
            bazaar_mass[j][0] = m;
            bazaar_mass[j][1] = n_val;
            ++j;
            printf("%d:%d ", m, n_val);
        }
        ++n_val;
    ...
    for (int i = 0; i < bazaar_size; ++i) {
        ...
        int m_val = bazaar_mass[p1][0];
        int n_val = bazaar_mass[p1][1];
        bazaar_mass[p1][0] = bazaar_mass[p2][0];
        bazaar_mass[p1][1] = bazaar_mass[p2][1];
        bazaar_mass[p2][0] = m_val;
        bazaar_mass[p2][1] = n_val;
    }
    printf("\nbazaar ready for game: ");
    for (int i = 0; i < bazaar_size; ++i)
    ...
    int result = arithmetic_sequence(n, f, s);
    if (result == n)
        printf("Yes;\n\n");
    else
        printf("No;\n\n");
    ...
    result = geometric_sequence(m, g, z);
    if (result == m)
        printf("Yes;\n\n");
    else
        printf("No;\n\n");
    ...
    precipitation[0] = 0;
    printf("Sequence: %d ", precipitation[0]);
    for (int i = 1; i <= 30; ++i) {
    ...
    while (precipitation[i] != 0) {
        printf("\nDay: %d, precipitation: %d;", i, precipitation[i]);
        ++i;
    }
    if (i == 30) {
