...
int gcd_multiple_numbers(int arr[], int size)
{
    int i = 0;
    int previous = arr[i];
    for (i = 1; i < size; ++i)
        previous = gcd(previous, arr[i]);
    return previous;
}

int reduce_fraction(int numerator, int denominator, int debug)
{
...
void print_date(int day, int month, int year, char* format)
{
    ...
    for (int i = 0, j = 0; format[i] != '\0'; ++i) {
        ...
        if (j != date_size && divs[j] > 0) {
            if (digits[j] >= fields[j] && j != date_size - 1) {
...
int quadratic_equations(int a, int b, int c)
{
    ...
    return (x1 != ceil(x1) || x2 != ceil(x1)) ? 1 : 0;
}
...
int is_command_correct(char command[], int idx, int pars, int src_size)
{
    ...
    for (int i = 1; i <= pars; ++i)
        if (hex_char_to_int(command[idx + i]) == -1)
            return 0;
    return 1;
}
...
void chapter_10()
{
    ...
    for (i = 0; i < parties; ++i) {
        ...
        P += line_vertex(&vertex[i * coordinates]);
    }
    printf("\np = %.2f;\n\n", P);
    ...
    printf(":(even): ");
    for (i = 0; i < SEQUENCE_LENGTH; ++i)
        printf("%d ", check_even_or_odd(0, sequence_1[i]));
    printf("\nsequence_2: ");
    for (i = 0; i < SEQUENCE_LENGTH; ++i)
    ...
    int arr_2[] = {25, 25, 125, 625, 725};
    int arr_int_size = sizeof(arr_2) / sizeof(int);
    printf("\n10.24 - 10.27, GCD and reduction of fractions;\n");
    printf("Reduction of fractions:\n");
    result = reduce_fraction(numerator, denominator, 1);
    printf("return = %d\n\n", result);
    printf("GCD for many numbers, using Evklid:\n");
    printf("numbers: ");
    for (i = 0; i < arr_int_size; ++i)
        printf("%d ", arr_2[i]);
    result = gcd_multiple_numbers(arr_2, arr_int_size);
    printf("\n\nsize of array: %d, GCD = %d\n\n", arr_int_size, result);
    char symbol = 'a';
    ...
    for (i = 0; i < texts_quant; ++i) {
        ...
        total_symb += quantity;
        int len_str = string_length(texts[i]);
        total_len += len_str;
        percent = (double)(quantity) / (double)(len_str) * 100;
        if (i != 0 && string_char_find(texts[i], symbol, 0, 0) < first)
            idx = i;
        first = string_char_find(texts[i], symbol, 0, 0);
        printf("ratio: %d/%d, in percentages: %.2f%%, ", quantity, len_str, percent);
    ...
    char commands[OBJ_MAX][STRING_MAX] = {"P06B80"};
    ...
    printf("\n10.33 - 10.37, drawing figures with command:\n");
    printf("P## - set position to coordinates x:y\n");
    ...
    for (k = 0; k < commands_size; ++k) {
        for (i = 0; commands[k][i] != '\0'; ++i) {
            switch (commands[k][i]) {
            case 'P':
                if (is_command_correct(commands[k], i, 2, screen_size) == 1) {
                    curr_x = hex_char_to_int(commands[k][i + 1]);
                    curr_y = hex_char_to_int(commands[k][i + 2]);
            ...
            case 'H':
                if (is_command_correct(commands[k], i, 1, screen_size)) {
                    int to_x = hex_char_to_int(commands[k][i + 1]);
                    int sign_h = (to_x < curr_x) ? -1 : 1;
                    printf("draw horizontal line from %d to %d\n", curr_x, to_x);
                    i += 1;
                    screen[curr_y * screen_size + to_x] = ink;
                    for (; to_x != curr_x; curr_x += sign_h)
                        screen[curr_y * screen_size + curr_x] = ink;
                } else
            ...
            case 'V':
                if (is_command_correct(commands[k], i, 1, screen_size)) {
                    int to_y = hex_char_to_int(commands[k][i + 1]);
                    int sign_v = (to_y < curr_y) ? -1 : 1;
                    printf("draw vertical line from %d to %d\n", curr_y, to_y);
                    i += 1;
                    screen[curr_y * screen_size + to_y] = ink;
                    for (; to_y != curr_y; curr_y += sign_v)
                        screen[curr_y * screen_size + curr_x] = ink;
                } else
                    printf("error at %d, parameter '%c'\n", i, commands[k][i + 1]);
                break;
            ...
            case 'B':
                if (is_command_correct(commands[k], i, 2, screen_size)) {
                    int to_x = hex_char_to_int(commands[k][i + 1]);
                    int to_y = hex_char_to_int(commands[k][i + 2]);
            ...
            case 'C':
                if (is_command_correct(commands[k], i, 1, screen_size)) {
                    radius = hex_char_to_int(commands[k][i + 1]);
    ...
    for (i = 0; i <= n; ++i)
        printf("%d ", numbers[i]);
    printf("\nmax = %d, ", find_max_recursive(numbers, n, &idx, max));
    printf("idx = %d;\n\n", idx);
    a1 = 0;
    int a2 = 3, a3 = 0, a4 = 3;
